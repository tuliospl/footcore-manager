...
const squadColumns = [
  ...
  { key: "overall", label: "Geral", direction: "desc" },
  { key: "potential", label: "Potencial", direction: "desc" },
  { key: "form", label: "Forma", direction: "desc" },
  { key: "appearances", label: "Jogos", direction: "desc" },
  { key: "goals", label: "Gols", direction: "desc" },
  { key: "assists", label: "Assistências", direction: "desc" },
...
function renderSquad() {
  ...
  content.innerHTML = `${pageHeading("Gestão esportiva", "Elenco", `${squad.length} no clube${ownedLoans.length ? ` · ${ownedLoans.length} emprestado${ownedLoans.length === 1 ? "" : "s"}` : ""}`)}
    ...
    ${squad.map(player => `<tr class="player-detail-row" data-player-details="${player.id}" data-player-return="squad" tabindex="0" aria-label="Abrir ficha de ${escapeHtml(player.name)}"><td class="player-name"><strong>${escapeHtml(player.name)}</strong>${loanDescription(player)}<small>${playerAvailabilityStatus(player, playerSquadRole(club, player, starters))} · Moral ${player.morale}</small><small class="discipline-status">${playerDisciplineStatus(player)}</small><small>${developmentLabel(player, game.season)}</small></td><td><span class="position-pill">${player.position}</span></td><td>${player.age}</td><td><span class="rating ${ratingClass(player.overall)}">${player.overall}</span></td><td>${player.potential}</td><td>${player.form}</td><td>${player.appearances}</td><td>${player.goals}</td><td>${player.assists ?? 0}</td><td>${formatMoney(player.value)}</td><td>${player.loan ? 'Empréstimo' : `Até o fim de ${contractLabel(player.contractEndSeason)}<small>${player.releaseClause ? `Multa: ${formatMoney(player.releaseClause)}` : "Sem multa rescisória"}</small>${player.contractEndSeason===game.season?'<small>Vence nesta temporada</small>':''}${player.contractEndSeason<game.season+2?`<button class="action-button" data-renew="${player.id}">Renovar · ${formatMoney(player.salary*4)}</button>`:''}`}</td><td>${player.loan ? `<span class="view-note">Empréstimo</span>` : `${player.academyGraduate && player.age <= 23 ? `<button class="action-button" data-loan-out="${player.id}">Emprestar</button>` : ""}<button class="action-button" data-sell="${player.id}">Vender</button>`}</td></tr>`).join("")}
...
function renderMarketResults() {
  ...
  document.querySelector("#market-results").innerHTML = entries.length ? entries.slice(marketPage * 60, (marketPage + 1) * 60).map(({ player, club, terms, listing }) => `<tr class="player-detail-row" data-player-details="${player.id}" data-player-return="market" tabindex="0" aria-label="Abrir ficha de ${escapeHtml(player.name)}">
    <td class="player-name"><strong>${escapeHtml(player.name)}</strong><small>${escapeHtml(club?.name || "Sem clube")}</small>${!club ? `<small>${player.freeAgentOrigin==='generated'?'Atleta fictício gerado pelo jogo':player.previousClub?`Contrato encerrado · ${escapeHtml(player.previousClub)}`:'Livre de contrato'}</small>` : ""}${terms ? `<small class="transfer-role">${terms.role}${terms.available ? "" : " · Indisponível"}</small>` : ""}</td>
...
function renderSeasonTeam(report) {
  ...
  const playerCard = player => `<article class="season-team-player" aria-label="${escapeHtml(player.playerName)}, ${player.position}, nota ${player.average.toFixed(1)}"><span class="pitch-shirt">${player.position}</span><strong>${escapeHtml(player.playerName)}</strong><small>${escapeHtml(player.clubName)}</small><em>Nota ${player.average.toFixed(1).replace(".", ",")}</em><span>${player.appearances}J · ${player.goals}G · ${player.assists}A</span></article>`;
  return `<section class="card season-team-card"><div class="card-header"><h3>Time do campeonato</h3><span>Formação 4-3-3 · melhor nota por posição</span></div><div class="season-team-pitch"><div class="field-markings" aria-hidden="true"><span class="field-center-circle"></span><span class="field-center-spot"></span><span class="field-box field-box-top"><i></i><b></b></span><span class="field-box field-box-bottom"><i></i><b></b></span></div>${positions.map(row => `<div class="season-team-row" style="--players:${row.length}">${row.map(playerCard).join("")}</div>`).join("")}</div></section>`;
}
