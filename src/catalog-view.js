...
export function renderCatalog(catalog, country, query, selection, busy, available = 20) {
  if (!catalog) return `<section class="card m26-intro"><div><h3>Seus times do pacote .m26</h3><p>Os elencos e escudos da pasta MKFP foram convertidos. Escolha os clubes que deseja usar.</p></div><button class="primary-button" data-import-action="catalog" ${busy ? "disabled" : ""}>Explorar times convertidos</button></section>`;
